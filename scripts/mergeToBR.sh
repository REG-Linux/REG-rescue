...
function checkFileToRemove() {
    ...
    if [[ ! -f ${BUILDROOT_DIR}/${file} ]]; then
        if (
            cd "${BUILDROOT_DIR}"
            git status --porcelain "${file}" | grep --silent "^ D "
        ); then
...
[[ -f ${hashFile} ]] &&
    ...
    {
        printError "${c_white}${hashFile}${c_reset} is missing"
        exit 1
    }
...
[[ -d ${BUILDROOT_DIR} && -d ${BUILDROOT_DIR}/package ]] &&
    ...
    {
        printError "${c_white}${BUILDROOT_DIR}${c_reset} is not a valid Buildroot tree"
        exit 1
    }
